...
export const http = defineConfig({
  /**
   * Trust the reverse proxy sitting in front of the app.
   *
   * Railway (and most PaaS providers) terminate TLS at their edge and forward
   * requests over an internal network with "X-Forwarded-*" headers. Trusting
   * the proxy ensures request.protocol() reports "https", secure cookies are
   * set correctly, and request.ip() reflects the real client address.
   *
   * The app is never exposed directly to the public internet on Railway, so
   * trusting every hop is safe here.
   */
  trustProxy: () => true,
